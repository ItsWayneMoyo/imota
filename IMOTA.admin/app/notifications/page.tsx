...
export default function NotificationsPage() {
  ...
  React.useEffect(() => {
    (async () => {
      try {
        const [mm, ff] = await Promise.all([metrics(), failures(100)]);
        setM(mm);
        setRows(Array.isArray(ff) ? ff : []);
      } catch (e) {
    ...
    <div className="grid gap-4">
      ...
      <div className="card p-4">
        ...
        <table className="table">
          <thead>
            <tr>
              <th>When</th><th>Channel</th><th>Target</th><th>Status</th>
              <th>Error</th><th>Attempts</th><th>Queue</th><th>Job</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: 12, color: '#666' }}>No failures found.</td></tr>
            ) : (
              rows.map((r: any) => (
                <tr key={r.id}>
